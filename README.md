...
这是 Windows 与 macOS Codex 桌面端的换肤引擎 2.0 版：不改任何官方文件，通过 Chromium DevTools Protocol（CDP）把皮肤"注入"到官方渲染器里，随时一键还原。Windows 与 macOS 都支持一键安装和“一张图自动生成主题”：脚本会自动取色、判断明暗路线并立即应用，不需要 AI。主题是纯数据（一个文件夹：`theme.json` + 一张图），而配套的 [THEME-SPEC.md](THEME-SPEC.md) 是一份**写给 AI agent 读的定制规范**——想精修时，把这个仓库和一张图丢给你的 Codex / Claude，它就能照着规范自己产出一套完整主题、自己截图调参、自己交付。你的 Codex，自己给自己换肤。
...
1. **两步冷启动**——Windows 使用 `quickstart.ps1` + `quick-theme.ps1`；macOS 双击安装后再双击选图，或运行统一命令。两端都能把任意图片自动取色、生成主题并立即点亮，全程不需要 AI。
...
**前置条件**：安装官方 Codex Mac 客户端，并至少正常打开、登录一次。AutoSkin 会使用 Codex 内置的 Node.js，不需要另外安装开发工具。

1. **下载并解压**——在 GitHub 页面选择 Code → Download ZIP，解压后在 Finder 打开 `codex-autoskin` 文件夹；
2. **双击安装**——打开 **`Install AutoSkin on macOS.command`**。如果 Codex 已在运行，安装器会询问是否重启；保存好正在编辑的内容后确认即可；
3. **用自己的图片生成主题**——打开 **`Create AutoSkin Theme on macOS.command`**，选择一张 PNG/JPG，程序会自动取色、生成并应用主题。也可以把图片直接拖到这个 `.command` 文件上。

> **不会删除记录**：普通安装使用原来的 Codex profile，不会清空 Projects、Tasks、登录状态或聊天记录。只有开发测试显式传入临时 `--user-data-dir` 时，才会看到一个没有历史记录的隔离实例。
>
> **建议先保留下载的文件夹**：安装后的运行文件不依赖源码仓库，但 Finder 的安装、生成主题和卸载入口目前仍在这个文件夹中。删除文件夹后功能仍可运行，只是需要使用下方的“安装后稳定入口”。

#### macOS 阻止打开时

如果提示“无法验证开发者”或不允许打开，先右键 `.command` 文件，选择**打开**，然后再次确认。若提示没有执行权限，在仓库目录打开终端并运行：

```bash
chmod +x ./*.command ./scripts/*.sh
```

如果右键打开仍被下载隔离阻止，并且确认文件来自本项目的 GitHub 仓库，可以运行：

```bash
xattr -dr com.apple.quarantine "/path/to/codex-autoskin"
```
...
如果 Codex 已经打开，安装 watcher 不会擅自打断当前实例，启动器也默认拒绝重启；确认可以重启时加 `--restart-existing`。非标准安装路径使用 `--app /path/to/ChatGPT.app`。

### macOS 使用说明（进阶）

应用默认位于 `/Applications/ChatGPT.app`，当前官方 bundle id 为 `com.openai.codex`；脚本也会检查 `~/Applications` 以及 `Codex.app` 名称。只有未来的 Codex 不再附带兼容运行时，脚本才会回退到系统中的 Node.js ≥ 20。
...
3. 把可独立运行的脚本、内置主题和素材复制到 `~/Library/Application Support/CodexDreamSkin/runtime`；个人生成的主题单独保存在旁边的 `themes-private`，更新运行文件时不会被覆盖；
4. 把所选端口、App 路径、PID、日志和配色备份保存在 `~/Library/Application Support/CodexDreamSkin`。

删除下载的仓库后，可以通过安装后的稳定入口继续使用：
...
"$HOME/Library/Application Support/CodexDreamSkin/runtime/scripts/autoskin-macos.sh" uninstall
```
...
# 从图片自动取色并生成主题；默认立即应用 fullscreen 版式
scripts/autoskin-macos.sh quick-theme "$HOME/Pictures/sunset.jpg" --name sunset

# 只生成、不实时应用，或改用横幅版式
scripts/autoskin-macos.sh quick-theme "$HOME/Pictures/sunset.jpg" --name sunset --no-apply
scripts/autoskin-macos.sh quick-theme "$HOME/Pictures/sunset.jpg" --name sunset --layout banner
...
Windows 的 `quick-theme.ps1` 与 macOS 的 `quick-theme` 命令都走最稳的通用参数：背景替换 + 基础配色。想更进一步——精调裁剪构图、卡片文案、装饰贴纸、特例样式——把这个仓库和你的图丢给你的 Codex / Claude，说：
...
之后正常启动 Codex 即为纯官方状态。macOS 默认保留 `~/Library/Application Support/CodexDreamSkin/themes-private` 中由你生成的个人主题，方便以后重新安装；不再需要时可手动删除这个目录。
...
For the easiest macOS setup: (1) install and sign in to the official Codex app once; (2) download/unzip this repo and right-click → Open `Install AutoSkin on macOS.command`; (3) open `Create AutoSkin Theme on macOS.command` and choose a PNG/JPG. Normal installation keeps the existing Codex profile and does not remove projects, tasks, chats, or login state. The equivalent terminal commands are `scripts/autoskin-macos.sh install` and `scripts/autoskin-macos.sh quick-theme /path/to/image.png --name my-theme`. Color extraction uses macOS's built-in image tools and the Node.js bundled with Codex, so no separate dependencies are normally needed. Installation copies a self-contained runtime to `~/Library/Application Support/CodexDreamSkin/runtime`; generated personal themes are kept separately under `themes-private`, so runtime updates do not overwrite them. Keep the downloaded folder if you want the Finder `.command` entry points; after deleting it, use the installed runtime command. The unified command also supports `theme`, `verify`, `doctor`, `restore`, and repeatable `uninstall`. If macOS blocks a downloaded script, use right-click → Open first. If screenshot capture is denied, grant Screen Recording permission to the terminal or agent running the command.
