...
安装完成后，双击 **`Create AutoSkin Theme on macOS.command`** 并在文件选择器里选一张 PNG/JPG，即可自动取色、生成并应用主题。也可以把图片直接拖到这个 `.command` 文件上。
