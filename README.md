...
**前置条件**：macOS 官方 Codex 桌面端、Node.js ≥ 20。应用默认位于 `/Applications/ChatGPT.app`，当前官方 bundle id 为 `com.openai.codex`；脚本也会检查 `~/Applications` 以及 `Codex.app` 名称。所有命令都应在仓库根目录运行。
...
3. 把 PID、日志和配色备份保存在 `~/Library/Application Support/CodexDreamSkin`。
...
# 端口被占用时，整套命令必须使用同一个备用端口
scripts/start-dream-skin.sh --port 19335
node scripts/set-theme.mjs ember-bloom fullscreen --port 19335
scripts/verify-dream-skin.sh --port 19335
scripts/restore-dream-skin.sh --port 19335
```
...
Codex 更新后不需要修改 app bundle；重新运行 `install-dream-skin.sh` 和 `start-dream-skin.sh` 即可重新发现当前应用并注入。
...
macOS prerequisites: the official Codex desktop app and Node.js ≥ 20. Run the scripts from the repository root. If Codex is already open, quit it first or explicitly pass `--restart-existing`; use `--app <path>` for a non-standard app location and `--no-auto-recover` to skip the LaunchAgent. If screenshot capture is denied, grant Screen Recording permission to the terminal or agent running the command. Runtime logs and the pre-install color backup live under `~/Library/Application Support/CodexDreamSkin`.
